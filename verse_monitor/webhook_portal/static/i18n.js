var I18N = (function(){
  ...
  var dict = {
    ...
    'reg.step2Ph':     { fr: 'https://votre-serveur.com/telegram-webhook', en: 'https://your-server.com/telegram-webhook' },
    'reg.step2Hint':   { fr: 'L\'URL HTTPS de VOTRE serveur qui recevra les alertes. Discord/Slack : collez l\'URL du webhook fourni par la plateforme. Telegram : créez un endpoint HTTPS sur votre serveur, puis utilisez l\'API setWebhook pour le lier à votre bot.', en: 'The HTTPS URL of YOUR server that will receive alerts. Discord/Slack: paste the webhook URL provided by the platform. Telegram: create an HTTPS endpoint on your server, then use the setWebhook API to link it to your bot.' },
    'reg.step2TelegramHint': { fr: '💡 Pour Telegram : cette URL doit être un endpoint HTTPS public sur VOTRE serveur. Verse Monitor enverra les alertes à cet endpoint, et votre bot les transmettra au chat. Utilisez ensuite l\'API Telegram setWebhook pour lier votre bot à cet endpoint.', en: '💡 For Telegram: this URL must be a public HTTPS endpoint on YOUR server. Verse Monitor will send alerts to this endpoint, and your bot will forward them to the chat. Then use the Telegram setWebhook API to link your bot to this endpoint.' },
    'reg.step3':       { fr: 'Format de sortie', en: 'Output Format' },
    'reg.step3Hint':   { fr: 'Discord, Slack, Telegram (Bot API), ou tout endpoint HTTPS. Pour autres plateformes, utilisez le format JSON générique avec un service bridge (Zapier, Make, n8n).', en: 'Discord, Slack, Telegram (Bot API), or any HTTPS endpoint. For other platforms, use the Generic JSON format with a bridge service (Zapier, Make, n8n).' },
