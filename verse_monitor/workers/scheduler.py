...
def build_scheduler(r: redis.Redis) -> AsyncIOScheduler:
    ...
    scheduler.add_job(
        ...
        seconds=settings.POLL_ROADMAP_INTERVAL,
        args=[r], max_instances=1, id="roadmap",
    )
    scheduler.add_job(
        devtracker.poll, "interval",
        seconds=settings.POLL_DEVTRACKER_INTERVAL,
        args=[r], max_instances=1, id="devtracker",
    )
    scheduler.add_job(
        comm_links.poll, "interval",
